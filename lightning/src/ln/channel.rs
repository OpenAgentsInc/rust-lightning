...
pub(super) struct ChannelContext<SP: SignerProvider> {
	...
	/// Counterparty local nonce advertised during simple-taproot channel establishment.
	simple_taproot_counterparty_initial_local_nonce: Option<Musig2PublicNonce>,
...
trait InitialRemoteCommitmentReceiver<SP: SignerProvider> {
	...
	fn check_counterparty_commitment_signature<L: Logger>(
		&self, sig: &Signature, simple_taproot_partial_signature_with_nonce: Option<&SimpleTaprootPartialSignatureWithNonce>,
		...
		self.context().simple_taproot_validate_commitment_partial(
			self.funding(),
	...
	fn initial_commitment_signed<L: Logger>(
		...
		let initial_commitment_tx = match self.check_counterparty_commitment_signature(&counterparty_signature, simple_taproot_partial_signature_with_nonce, holder_commitment_point, logger) {
			Ok(res) => res,
...
impl<SP: SignerProvider> ChannelContext<SP> {
	fn new_for_inbound_channel<'a, ES: EntropySource, F: FeeEstimator, L: Logger>(
		...
		let simple_taproot_counterparty_initial_local_nonce = if channel_type
			.requires_simple_taproot()
			|| channel_type.requires_simple_taproot_staging()
			|| channel_type.requires_taproot_asset_channel()
		{
			open_channel_fields.simple_taproot_next_local_nonce
		} else {
			None
		};
		...
		let channel_context = ChannelContext {
			...
			simple_taproot_counterparty_initial_local_nonce,
	...
	fn new_for_outbound_channel<'a, ES: EntropySource, F: FeeEstimator, L: Logger>(
		...
		let channel_context = Self {
			...
			funding_locked_txid_sent_in_reestablish: None,
			simple_taproot_nonce_state: SimpleTaprootNonceState::new(),
			simple_taproot_counterparty_initial_local_nonce: None,
			simple_taproot_counterparty_next_local_nonces: SimpleTaprootNonceEntries::default(),
			simple_taproot_sent_commitment_signatures:
				SimpleTaprootSentCommitmentSignatures::default(),
			simple_taproot_local_closee_nonce_index: 0,
	...
	pub fn do_accept_channel_checks(
		...
		if Self::is_simple_taproot_funding(funding) {
			self.simple_taproot_counterparty_initial_local_nonce =
				common_fields.simple_taproot_next_local_nonce;
		}
	...
	fn simple_taproot_initial_nonce_txid(&self) -> Result<Txid, ChannelError> {
		self.temporary_channel_id.map(|channel_id| Txid::from_byte_array(channel_id.0)).ok_or_else(
			|| {
				ChannelError::close(
					"Missing temporary channel id for simple-taproot initial nonce".to_owned(),
				)
			},
		)
	}
	...
	#[cfg(feature = "simple_taproot_musig2")]
	fn simple_taproot_initial_nonce_counterparty_funding_pubkey(
	...
	#[cfg(feature = "simple_taproot_musig2")]
	fn simple_taproot_initial_local_public_nonce(
		&self, funding: &FundingScope, nonce_index: u64,
	) -> Result<Option<Musig2PublicNonce>, ChannelError> {
		if !Self::is_simple_taproot_funding(funding) {
			return Ok(None);
		}
		let nonce_txid = self.simple_taproot_initial_nonce_txid()?;
		...
		self.holder_signer
			...
			.map_err(|err| {
				ChannelError::close(format!(
					"Failed to derive simple-taproot initial nonce: {:?}",
					err
				))
			})
	}
	...
	fn simple_taproot_sign_commitment_partial(
		...
		let counterparty_public_nonce = if nonce_index == INITIAL_COMMITMENT_NUMBER {
			self.simple_taproot_counterparty_initial_local_nonce.ok_or_else(|| {
				ChannelError::close(
					"Missing simple-taproot counterparty initial local nonce".to_owned(),
				)
			})?
		} else {
			self.simple_taproot_counterparty_next_local_nonces.get(funding_txid).ok_or_else(
				|| {
					ChannelError::close(
	...
	> {
		...
		let commitment_txid = {
			...
			if !ChannelContext::<SP>::is_simple_taproot_funding(funding) {
				let sighash =
					bitcoin_tx.get_sighash_all(&funding_script, funding.get_value_satoshis());
			...
			commitment_data.tx.nondust_htlcs().iter().zip(msg.htlc_signatures.iter())
		{
			...
			let mut adjusted_htlc = None;
			#[cfg(feature = "simple_taproot_musig2")]
			if chan_utils::requires_simple_taproot_outputs(funding.get_channel_type())
				&& funding.get_channel_type().requires_taproot_asset_channel()
			{
				if let Some((htlc_id, htlc_blob)) =
					self.taproot_asset_htlc_blob_for_commitment_htlc(htlc, true, false)
				{
					if let Some(second_level_aux_leaf) = self
						...
						let mut htlc_with_exact_second_level = htlc.clone();
						htlc_with_exact_second_level.simple_taproot_second_level_aux_leaf_script =
							Some(second_level_aux_leaf);
						adjusted_htlc = Some(htlc_with_exact_second_level);
					}
				}
			}
			let htlc_for_signature = adjusted_htlc.as_ref().unwrap_or(htlc);
	...
	#[cfg(feature = "simple_taproot_musig2")]
	fn taproot_asset_htlc_blob_for_commitment_htlc<'a>(
		&'a self, htlc: &HTLCOutputInCommitment, local: bool, generated_by_local: bool,
	) -> Option<(u64, &'a Vec<u8>)> {
		for pending in self.pending_inbound_htlcs.iter() {
			if pending.state.included_in_commitment(generated_by_local)
				&& (false == local) == htlc.offered
				&& pending.amount_msat == htlc.amount_msat
				&& pending.cltv_expiry == htlc.cltv_expiry
				&& pending.payment_hash == htlc.payment_hash
			{
				return pending
					.taproot_asset_htlc_blob
					.as_ref()
					.map(|blob| (pending.htlc_id, blob));
			}
		}

		for pending in self.pending_outbound_htlcs.iter() {
			if pending.state.included_in_commitment(generated_by_local)
				&& (true == local) == htlc.offered
				&& pending.amount_msat == htlc.amount_msat
				&& pending.cltv_expiry == htlc.cltv_expiry
				&& pending.payment_hash == htlc.payment_hash
			{
	...
	#[cfg(feature = "simple_taproot_musig2")]
	fn compressed_even_key_from_xonly(output_xonly_key: [u8; 32]) -> [u8; 33] {
		let mut script_key = [0u8; PUBLIC_KEY_SIZE];
		script_key[0] = 0x02;
		script_key[1..].copy_from_slice(&output_xonly_key);
		script_key
	}

	#[cfg(feature = "simple_taproot_musig2")]
	fn taproot_asset_htlc_asset_script_key(
		&self, funding: &FundingScope, per_commitment_point: &PublicKey, local: bool, htlc_id: u64,
		offered: bool, cltv_expiry: u32, payment_hash: &PaymentHash,
		...
		Some(Self::compressed_even_key_from_xonly(output_xonly_key))
	}
	...
	#[cfg(feature = "simple_taproot_musig2")]
	fn taproot_asset_second_level_htlc_aux_leaf_script_for_commitment_output(
		&self, funding: &FundingScope, per_commitment_point: &PublicKey, local: bool,
		htlc_blob: &Vec<u8>, htlc_id: u64, offered: bool, cltv_expiry: u32,
		payment_hash: &PaymentHash, commitment_txid: Txid, htlc_output_index: u32,
	) -> Option<ScriptBuf> {
		if !funding.get_channel_type().requires_taproot_asset_channel() {
			return None;
		}
		...
		let previous_script_key = self.taproot_asset_htlc_asset_script_key(
			funding,
			per_commitment_point,
			local,
			htlc_id,
		...
		let previous_output_template = template
			.with_previous_asset_output(
				commitment_txid.to_byte_array(),
				htlc_output_index,
				previous_script_key,
				None,
			)
	...
	fn get_funding_signed_msg<L: Logger>(
		...
		counterparty_initial_commitment_number: u64,
		...
		let simple_taproot_partial_signature_with_nonce = match self
			.simple_taproot_sign_commitment_partial(
				funding,
				counterparty_initial_commitment_number,
				&counterparty_initial_bitcoin_tx.transaction,
			) {
			Ok(partial_signature) => partial_signature,
			Err(err) => {
				log_trace!(logger, "Counterparty commitment simple-taproot signature not available for funding_signed message: {}", err);
				self.signer_pending_funding = true;
				return None;
			},
		};
	...
	SP::EcdsaSigner: EcdsaChannelSigner,
{
	...
	pub fn signer_maybe_unblocked<L: Logger, CBP>(
			...
			&& self.pending_splice.is_none()
		{
			...
			let counterparty_initial_commitment_tx = commitment_data.tx;
			self.context.get_funding_signed_msg(&self.funding, logger, counterparty_initial_commitment_tx, self.context.counterparty_next_commitment_transaction_number + 1)
		} else { None };
...
impl<SP: SignerProvider> OutboundV1Channel<SP> {
	...
	fn get_funding_created_msg<L: Logger>(&mut self, logger: &L) -> Option<msgs::FundingCreated> {
		...
		let simple_taproot_partial_signature_with_nonce = match self.context.simple_taproot_sign_commitment_partial(
			&self.funding,
			self.context.counterparty_next_commitment_transaction_number,
			&counterparty_initial_commitment_tx.trust().built_transaction().transaction,
		) {
			Ok(partial_signature) => partial_signature,
			Err(err) => {
				log_trace!(logger, "funding_created awaiting simple-taproot signature: {}", err);
				self.context.signer_pending_funding = true;
				return None;
			},
		};
	...
	pub fn get_open_channel<L: Logger>(
		...
		let simple_taproot_next_local_nonce = match self.context.simple_taproot_initial_local_public_nonce(
			&self.funding,
			self.unfunded_context.transaction_number(),
		) {
			Ok(nonce) => nonce,
			Err(err) => {
				log_trace!(_logger, "Unable to generate open_channel message, waiting for simple-taproot nonce: {}", err);
				self.signer_pending_open_channel = true;
				return None;
			},
		};
...
impl<SP: SignerProvider> InboundV1Channel<SP> {
	...
	fn generate_accept_channel_message<L: Logger>(
		...
		let simple_taproot_next_local_nonce = match self.context.simple_taproot_initial_local_public_nonce(
			...
			Ok(nonce) => nonce,
			Err(err) => {
				log_trace!(_logger, "Unable to generate accept_channel message, waiting for simple-taproot nonce: {}", err);
				self.signer_pending_accept_channel = true;
				return None;
			},
		};
...
impl<SP: SignerProvider> Writeable for FundedChannel<SP> {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		let simple_taproot_counterparty_initial_local_nonce =
			self.context.simple_taproot_counterparty_initial_local_nonce.as_ref();
	...
	ReadableArgs<(&'a ES, &'b SP, &'c ChannelTypeFeatures)> for FundedChannel<SP>
{
	fn read<R: io::Read>(
		...
		let mut simple_taproot_counterparty_initial_local_nonce: Option<Musig2PublicNonce> = None;
