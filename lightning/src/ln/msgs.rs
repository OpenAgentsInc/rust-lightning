...
pub struct UpdateAddHTLC {
	...
	/// Opaque Taproot Assets HTLC data carried by Lightning Labs' experimental asset-channel
	/// implementation.
	///
	/// This is a required even TLV on the wire, so nodes that advertise Taproot Asset channel
	/// support must recognize and preserve it even before validating the asset proof semantics.
...
impl_writeable_msg!(UpdateAddHTLC, {
	...
	(65536, taproot_asset_htlc_blob, (option, encoding: (Vec<u8>, WithoutLength))),
...
mod tests {
	...
	#[test]
	fn update_add_htlc_taproot_asset_blob_round_trips_required_tlv() {
		let blob = <Vec<u8>>::from_hex(
			"012c00204ab5ffbe793a4f8a2552d262a1dc8ef8da8d4768605da68460b2d1f5e70a575f01080000000000000001",
		)
		.unwrap();
		let mut msg = test_update_add_htlc();
		msg.taproot_asset_htlc_blob = Some(blob.clone());

		let encoded = msg.encode();
		let mut expected_suffix = Vec::new();
		append_tlv(&mut expected_suffix, 65536, &blob);
		assert!(encoded.ends_with(&expected_suffix));

		let decoded: msgs::UpdateAddHTLC =
			LengthReadable::read_from_fixed_length_buffer(&mut &encoded[..]).unwrap();
		assert_eq!(decoded.taproot_asset_htlc_blob, Some(blob));
	}
